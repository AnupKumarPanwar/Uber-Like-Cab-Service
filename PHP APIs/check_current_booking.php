...
$con=mysqli_connect($dbServer,$dbUsername,$dbPassword,$dbName);
	...
	if (mysqli_num_rows($result)>0) {
		...
		$response=array(
			"status"=>"1",
			"data"=> array("customerName" => $r['user_name'], "customerPhone"=>$r['customer_phone'], "src_lat" => $r['src_lat'], "src_lng" => $r['src_lng'], "dest_lat"=>$r['dest_lat'], "dest_lng" => $r['dest_lng'], "fare" => $r['fare'], "otp" => $r['otp'], "ride_id" => $r['ride_id'], "cab_id" => $r['cab_id'])
			);
		die(json_encode($response));	
	}
	else
