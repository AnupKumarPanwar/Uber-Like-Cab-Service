<?php
	...
	$getNearbyCabs="SELECT driver_id, driver_name, phone, cab_id, cabs.cab_no cab_no, cab_lat, cab_lng FROM cabs, drivers WHERE on_trip=0 and on_duty=1 and drivers.cab_no=cabs.cab_no ORDER BY ((cab_lat-$src_lat)*(cab_lat-$src_lat) + (cab_lng-$src_lng)*(cab_lng-$src_lng)) LIMIT 1";
	...
	if ($result) {
		...
		$otp=rand(1000, 9999);
		$fare=sqrt(($dest_lat-$src_lat)*($dest_lat-$src_lat)+($dest_lng-$src_lng)*($dest_lng-$src_lng))*111*80;
		$fare=(int)$fare;
		// echo($fare);
		// echo('<br>'.$otp);
